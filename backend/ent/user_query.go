...
func (uq *UserQuery) QueryTitle() *TitleQuery {
	query := &TitleQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithTitle(opts ...func(*TitleQuery)) *UserQuery {
	query := &TitleQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if uq.withGender != nil || uq.withPosition != nil || uq.withTitle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withTitle; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
